const Carousel = ({ darkMode }) => {
  ...
  const carouselItems2 = [
    ...
    { name: "Laravel", src: "https://laravel.com/img/logomark.min.svg" },
    { name: "Tailwind", src: "https://unpkg.com/simple-icons@v8/icons/tailwindcss.svg" },
  ...
  return (
    <section id="Skills" className={`mt-8 rounded-lg shadow-lg p-6 transition-colors duration-500 ${darkMode ? "bg-gray-900 text-gray-100" : "bg-white text-gray-900"}`}>
      ...
      <div className="flex flex-col gap-6 overflow-hidden">
        {/* Carousel 1 */}
        <div className={`flex gap-12 animate-scrollRight whitespace-nowrap rounded-lg p-4 shadow-inner transition-colors duration-300 ${darkMode ? "bg-gray-800" : "bg-gray-100"}`} style={{ animationDuration: "20s", willChange: "transform" }}>
          {[...carouselItems1, ...carouselItems1].map(({ name, src }, i) => (
            <div key={name + i} className={`min-w-[150px] max-w-[180px] p-6 rounded-xl shadow-md flex flex-col items-center justify-center transition-colors duration-300 ${darkMode ? "bg-blue-900 text-gray-100" : "bg-blue-100 text-gray-900"}`}>
              <img src={src} alt={name} title={name} className="w-12 h-12" />
